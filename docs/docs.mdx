...
`Mask` wraps any content with a radial spotlight. Enable cursor tracking with `withCursorMask` or pin the mask using `maskX`, `maskY`. Use `maskRadius` (or `maskRadiusX`/`maskRadiusY`) and `maskBackground` for custom looks.

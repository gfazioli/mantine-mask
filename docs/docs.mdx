...
`Mask` wraps any content (for example `JsonTree`) with a radial spotlight mask. You can let the mask follow the cursor (`withCursorMask`) or pin it to static coordinates with `maskX`, `maskY`, and a configurable `maskRadius`. The surrounding area is dimmed with `maskBackground`, making the focused region easier to read during demos or presentations.
